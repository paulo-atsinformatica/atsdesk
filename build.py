...
hbb_name = 'atsdesk' + ('.exe' if windows else '')
...
def build_flutter_windows(version, features, skip_portable_pack):
    ...
    if os.path.exists('./atsdesk_portable.exe'):
        os.replace('./target/release/atsdesk-portable-packer.exe',
                   './atsdesk_portable.exe')
    else:
        os.rename('./target/release/atsdesk-portable-packer.exe',
                  './atsdesk_portable.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/atsdesk_portable.exe')
    os.rename('./atsdesk_portable.exe', f'./atsdesk-{version}-install.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/atsdesk-{version}-install.exe')
...
def main():
    ...
    if windows:
        ...
        # system2('upx.exe target/release/atsdesk.exe')
        system2('mv target/release/atsdesk.exe target/release/ATSDesk.exe')
        pa = os.environ.get('P')
        if pa:
            # https://certera.com/kb/tutorial-guide-for-safenet-authentication-client-for-code-signing/
            system2(
                f'signtool sign /a /v /p {pa} /debug /f .\\cert.pfx /t http://timestamp.digicert.com  '
                'target\\release\\atsdesk.exe')
        else:
            print('Not signed')
        system2(
            f'cp -rf target/release/ATSDesk.exe {res_dir}')
        os.chdir('libs/portable')
        system2('pip3 install -r requirements.txt')
        system2(
            f'python3 ./generate.py -f ../../{res_dir} -o . -e ../../{res_dir}/atsdesk-{version}-win7-install.exe')
        system2(f'mv ../../{res_dir}/atsdesk-{version}-win7-install.exe ../..')
